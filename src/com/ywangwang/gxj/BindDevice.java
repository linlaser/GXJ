...
public class BindDevice extends Activity {
			...
			if (msg.what == GlobalInfo.HANDLER_DELAY_BROADCAST_BIND_DEVICE) {
				...
				if (bindSubDevice_temp.used) {
					if (times > 5) {
						...
						newDialog.show(BindDevice.this, "绑定失败", "绑定失败！");
					} else {
					...
					CustomDialog newDialog = new CustomDialog();
					newDialog.show(BindDevice.this, "绑定成功", "绑定成功！");
				}
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CustomDialog newDialog = new CustomDialog();
		newDialog.show(BindDevice.this, "说明", "长按净水器的冲洗键5~10秒，听到嘀..嘀..嘀..三声，\n净水器就可进入被搜索状态。");
	}
	...
	OnItemClickListener boundDeviceListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			CustomDialog newDialog = new CustomDialog();
			newDialog.show(BindDevice.this, "取消绑定设备", "确认取消绑定净水器【" + GlobalInfo.boundJSQ.getDeviceType() + " (" + GlobalInfo.boundJSQ.add + ")】？\n取消绑定后之前的净水数据将被清除。", "取消", "确定", null, runnableUnBindNewJSQDeivce());
		}
	};
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			if (position < searchedDevice.length) {
				if (searchedDevice[position].add == GlobalInfo.boundJSQ.add && GlobalInfo.boundJSQ.used == true) {
					...
					newDialog.show(BindDevice.this, "绑定设备", "当前设备已经绑定了净水器【" + GlobalInfo.boundJSQ.getDeviceType() + " (" + GlobalInfo.boundJSQ.add + ")】，\n如果重新绑定新的净水器【" + searchedDevice[position].getDeviceType() + " (" + searchedDevice[position].add + ")】，之前的净水数据将被清除。\n请确认是否绑定？", "取消", "确定", null, runnableBindNewJSQDeivce());
				} else if (GlobalInfo.boundJSQ.used == false) {
					listViewPosition = position;
					newDialog.show(BindDevice.this, "绑定设备", "确认绑定新的净水器【" + searchedDevice[position].getDeviceType() + " (" + searchedDevice[position].add + ")】？", "取消", "确定", null, runnableBindNewJSQDeivce());
				}
			}
		}
	};
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		MenuItem reSearch = menu.add(0, RESEARCH, 0, "重新搜索");
