...
{
    int status = MI_ERR;
    unsigned char t_buf[2] = {0,0,};
    unsigned char r_buf[2] = {0,0,};
	...
	struct spi_ioc_transfer tr = {
		.tx_buf = (unsigned long)t_buf,
		.rx_buf = (unsigned long)r_buf,
		.len = 2,
	};
	status = ioctl(fd, SPI_IOC_MESSAGE(1), &tr);
	if (status < 1)pabort("can't read byte form device");
...
void calulatate_crc (unsigned char *indata, unsigned char len, unsigned char *outdata)
{
    ...
    for(i=0; i<len; i++){
        write_reg(FIFO_DATA_REG, *(indata+i));
    }
 ...
 * Return Value:
 *************************************************************************************/
void spi_init(int fd, unsigned char mode, unsigned char bits,
		unsigned int speed) {
	int ret;
...
//	unsigned char WriteData[16] = {5, 6, 7, 0xef, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 6, 5,4};
...
//
//while(1){
//	status= rfid_request(PICC_REQALL,CardType);					//寻卡，CardType同时输出卡类型代码
//	if(status!=MI_OK){status = rfid_request(PICC_REQALL,CardType);}			//如果卡已经放到读卡器上，第一次寻卡失败，马上再寻卡一次肯定成功。
//	//if(status!=MI_OK){printf("--------rfid_request  ERR------\n");}				//没有寻到卡，或寻卡失败
//	if(status==MI_OK){status = rfid_anticoll(CardID);}			  		//放冲突，CardID同时输出卡序列号
//	//if(status!=MI_OK)printf("--------rfid_anticoll  ERR------\n");
//	if(status==MI_OK){status = rfid_select(CardID);}					//选卡，输入卡序列号
//        //if(status!=MI_OK)printf("--------rfid_select  ERR------\n");
//	if(status==MI_OK){status = rfid_auth_state(PICC_AUTHENT1A,1, DefaultKey, CardID);}	//认证，校验卡密码
//	//if(status==MI_OK){status = rfid_write(1,WriteData);}				//写数据
//	if(status==MI_OK){status = rfid_read(1,ReadData);}				//读数据
//	if (status == 0){
//		int i;
//		printf("CardType=");
//		for(i=0;i<sizeof(CardType);i++){
//			printf("%.2X ",CardType[i]);
//		}
//		puts("");
//		printf("CardID=");
//		for(i=0;i<sizeof(CardID);i++){
//			printf("%.2X ",CardID[i]);
//		}
//		puts("");
//		printf("ReadData=");
